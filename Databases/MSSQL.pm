...
sub _getDatabaseService {
    ...
    foreach my $credential (@{$credentials}) {
        ...
        $starttime =~ s/\..*$// if defined($starttime);
        ...
        foreach my $db (_runSql(
            ...
            # Always On secondary replicas / offline DBs may not respond to
            # sp_spaceused or sys.objects — guard against undef results.
            my $size;
            my $spaceused = _runSql(
                sql => "USE [$db_name] ; EXEC sp_spaceused",
                %params
            );
            if (defined($spaceused) && $spaceused =~ /^$db_name;([0-9.]+\s*\S+);/) {
